...
void Image::drawCircle(Vector2 & centre, Vector2 & r, int plus, const Color& c) {
	int i = 0;
	while (i <= plus) {
		for (float y = 0; y < 2 * PI; y += 0.005)
		{
			setPixelSafe(cos(y)*int((r.distance(centre)+i)) + centre.x, sin(y)*int(r.distance(centre)+i) + centre.y, c); //draw cirecle
		}
		i++;
	}
}
